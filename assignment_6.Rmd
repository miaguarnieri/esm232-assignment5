...
simtimes <- seq(from = 1, to = 300)
parms <- list(r = sens_C$X[, "r"], K = sens_C$X[, "K"], g = sens_C$X[, "g"], closure = sens_C$X[, "closure"])

#turn computing our metrics into a function
compute_metrics <- function(result) {
  maxforest <- max(result$C)
  idx <- which(result$C == max(result$C))[1]
  idx <- ifelse(is.na(idx), length(result$C), idx)
  maxyear = result$time[idx]
  return(list(maxforest = maxforest, maxyear = maxyear))}
